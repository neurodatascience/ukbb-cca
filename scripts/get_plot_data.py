...
from aggregate_sample_size_results import (
    find_fpaths_results,
    combine_results,
    RE_SAMPLE_SIZE,
)
...
@click.option(
    ...
    default=["summary"],
    multiple=True,
    type=click.Choice(VALID_RESULT_TYPES),
)
    ...
    for requested_result_type in requested_result_types:
        if requested_result_type == "summary":
                ...
                for set_name in data.levels["set_name"]:
                    for func in data.levels["func"]:
                        for i_dataset in range(len(data.dataset_names)):
                            data[cca_type, set_name, func].loadings[i_dataset].index = (
                                db_helper.udis_to_text(
                                    data[cca_type, set_name, func]
                                    .loadings[i_dataset]
                                    .index,
                                    encoded=True,
                                    prepend_category=True,
                                )
                            )

            data.fname = add_suffix(
                requested_result_type, add_suffix(summary.fname, sample_size)
            )
            ...
            if sample_size is None:
                sample_size = max(
                    [
                        int(re.match(RE_SAMPLE_SIZE, dname.name).group(1))
                        for dname in dpath_subset.iterdir()
                        if dname.is_dir()
                    ]
                )
                print(f"Using largest sample size: {sample_size}")
            ...
            fpaths_results, i_reps = find_fpaths_results(
                dpath_results, return_i_reps=True, rep_label=rep_label
            )
            ...
            for i_dataset, dataset_name in enumerate(results_combined.dataset_names):
                ...
                for result in results_combined[(sample_size, cca_type, set_name)]:
                    if requested_result_type == "corrs":
                        requested_result = {
                            f"CA{i_component+1}": getattr(
                                result, requested_result_type
                            )[i_component]
                        }
                    else:
                        dataset_results = getattr(result, requested_result_type)[
                            i_dataset
                        ]
                ...
                df = pd.DataFrame(data_for_df, index=i_reps)

                if requested_result_type == "loadings":
                    df.columns = db_helper.udis_to_text(
                        df.columns, encoded=True, prepend_category=True
                    )

                fpath_out_components = [
                ...
                if requested_result_type == "corrs":
                    fpath_out_components = fpath_out_components[:-1]
                fpath_out = dpath_out / f'{"-".join(fpath_out_components)}.csv'
                df.to_csv(fpath_out, index=True, header=True)
                print(f"Saved to {fpath_out}")

                if requested_result_type == "corrs":
                    break  # same result for all datasets for corr

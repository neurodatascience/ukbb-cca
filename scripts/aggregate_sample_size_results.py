...
def aggregate_sample_size_results(n_pcs_all, dpath_cca, subset, i_component, is_test, use_largest_null_model):
    ...
    if results_combined_null is not None:
    ...
    # plot
    largest_sample_size = sorted(results_summary.levels['sample_size'])[-1]
    for sample_size in results_summary.levels['sample_size']:
        ...
        for i_row, cca_type in enumerate(cca_types):
            ...
            if results_summary_null is not None:
                try:
                    bootstrap_corrs = [
                        results_summary_null[sample_size_null_model][cca_type]['val'][f'quantile_{quantile}'].corrs
        ...
        for cca_type in results.method_names:
            ...
            for set_name in results[cca_type].set_names:
                try:
                    ...
                    if (n_components_expected is not None) and (len(single_result.corrs) != n_components_expected):
                        print('====================')
